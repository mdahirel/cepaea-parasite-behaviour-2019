...
First, our food intake submodel. Food intake is a proportion of the total food given, so a Beta model should be natural. There are a few zeroes inside which could be a problem, but we can solve that by setting them to the detection limit of our balance and using left-censoring (so we reinterpret the observed zeroes as actually "below the 0.0001g detection threshold"). Let's try:
...
- we re-include the effect of population in the nematode sub-model
