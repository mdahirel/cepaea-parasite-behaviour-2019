---
title: 'main analysis code for "Morph-dependent nematode infection and its association with host movement in the land snail *Cepaea nemoralis* (Mollusca, Gastropoda)"'
author: "Maxime Dahirel, Marine Proux, Claudia Gérard, Armelle Ansart (this code by M. Dahirel)"
date:
output: 
  html_document:
    theme: yeti
...
This code is used to analyze data collected during Marine Proux's internship in 2019. This was a study of how different morphs of *Cepaea nemoralis* varied in their behaviour and in the parasites they naturally harbour, and whether there was a link between parasitism and behaviour (see more details in the manuscript). 180 snails were tested, minus one that died during the experiments (it is still in the source dataset because it is still useful in some ways, see below for how we handle that)
...
- `Food_intake`: Quantity of food consumed (in g, out of 1.5 g) during food intake tests.
...
In the new dataset, we then center and standardize the continuous variables. We do that in two steps. First, the easy ones, so everything but movement:
...
We then prep the movement variable, which is a bit more complex. We first need to create our actual "latency to move away" variable, by removing the time unactive `FPT1` from the total response times `FPT3` before using them. We need to do that properly in particular for the individuals that did not move at all (because a naive `FPT3 - FPT1` when both are 1200sec (the maximum) gives 0, which is the wrong answer). Finally, as the survival model on the untransformed latencies doesn't work well (see below), we'll also prepare a variable that is the inverse latency to analyse with a Gaussian model later (so we'll center and scale it too). 
...
data %>%
  group_by(fullID) %>%
  summarise(Nvalid_obs = sum(is_active)) %>%
  select(Nvalid_obs) %>%
  table()
```
...
#how many snails are infected by nematodes in each population?
table(data_parasite$Nematode_live_all>0,data_parasite$population)
...
subset(data_parasite, data_parasite$Acari_live_all > 0)$Acari_live_all
# or that encapsulated some?
subset(data_parasite, data_parasite$Acari_encaps > 0)
subset(data_parasite, data_parasite$Acari_encaps > 0)$Acari_encaps
...
cont_table
cont_table[1,2]/sum(cont_table[,2])
```

We can also (restarting from the raw data) get some info about parasite typical size, since we collected them by size class. We must note however that size was not exactly measured in this protocol, meaning nematodes may be attributed wrongly to the next smaller or larger class. As such, we advise against using these data for anything other than broad inference about the most common size class(es)

```{r nematode-size}
# create a table where one row = one live nematode, with attributes snail ID, whether the snail was dead before end of experiment, organ the nematode was found in and nematode size class
nematode_size <- raw_parasite %>% 
  select(fullID,dead,Nematode_0_1_free,Nematode_1_3_free,Nematode_3_5_free,Nematode_5_plus_free,
         Nematode_1_3_lung,Nematode_3_5_lung,Nematode_5_plus_lung) %>% 
  pivot_longer(cols=c(Nematode_0_1_free:Nematode_5_plus_lung),values_to = "N") %>% 
  mutate(organ=case_when(is.na(str_match(name,"free"))~"lung",
                         T~"free")) %>% 
  mutate(sizeclass1=str_split_fixed(name,"_",4)[,2],
         sizeclass2=str_split_fixed(name,"_",4)[,3]) %>% 
  mutate(sizeclass=paste(sizeclass1,sizeclass2,sep="_")) %>% 
  select(fullID,dead,N,organ,sizeclass) %>% 
  mutate(sizeclass=ordered(sizeclass,levels=c("0_1","1_3","3_5","5_plus"))) %>% 
  uncount(weights=N)


# give the proportions of nematodes found per size class
table(nematode_size$sizeclass)
sum(table(nematode_size$sizeclass))
table(nematode_size$sizeclass)/sum(table(nematode_size$sizeclass))

#split the table by snail was alive versus snail was dead
by_dead=table(nematode_size$sizeclass,nematode_size$dead)

# give the proportions of nematodes found per size class
by_dead
sum(by_dead[,1])
by_dead[,1]/sum(by_dead[,1])
```
...
```{r model-size}
if (file.exists(here("R_output", "model_size.RDS")))
...
# to override, re-run the model and re-save manually by selecting relevant code lines then knit (or delete the RDS object)
  {
    mod_size <- readRDS(here("R_output", "model_size.RDS"))
  } else {
  mod_size <- brm(
  ...
  saveRDS(mod_size, file = here("R_output", "model_size.RDS"))
}
```
...
(skip to 2D if you don't need/ want to run the tests, and want to go to the actual model)

OK, before building the big multivariate model, let's check that the families we have in mind for each submodel are fine. The trematode and nematode submodels should be OK (we'll still check them later in the full model), let's focus our effort on the movement and food intake model.

First, our food intake submodel. Food intake is a proportion of the total food given, so a Beta model should be natural. There are a few zeroes inside which could be a problem, but we can solve that by setting them to the detection limit of our scale and using left-censoring (so we reinterpret the observed zeroes as actually "below the 0.0001g detection threshold"). Let's try:
...
mod_food_test_beta <- brm(
  bf(food_test / 1.5 | cens(-1 * (food_test <= 0.0001)) ~ # we use left-censoring (-1) to allow zero values in a Beta model
...
mod_mvt_test_lognormal <- brm(
  bf(latency | subset(is_active == TRUE) + ## we exclude non-active snails
...
It's important (and reassuring) to see that the key results from the fixed effects are kept whatever the implementation of the model :)
...
```{r multi-model}
if (file.exists(here("R_output", "model_multi.RDS")))
# this if-else statement is avoid re-fitting a model when knitting Rmd file if there is already one existing in R_output
# to override, re-run the model and re-save manually by selecting relevant code lines then knit (or delete the RDS object)
  {
  ...
  saveRDS(mod_multi, file = here("R_output", "model_multi.RDS"))
}
...
## because no rows containing an NA have been automatically discarded
```
...
Now we can move to building the tables and the figures. But before, we can...

## 2E - ...check that correlations involving nematode do not change whether or not we exclude the nematode-free population

(you can skip directly to Part 3 if you don't want to re-run the model below)

To do that, we re-fit the multivariate model, with 2 changes:

- we do not exclude anymore the snails from the shaded population from the nematode sub-model

- as a consequence, we re-include the effect of population in the nematode sub-model

Other than that, everything else is unchanged:

```{r}
bf_nema_live2 <- bf(Nematode_live_all | subset(session == 1) ~
0 + band_number + band_number:population + scale_size +
  (1 | series) + (1 | q | fullID),
family = poisson
)

mod_multi2 <- brm(mvbf(bf_food + bf_trema + bf_nema_live2 + bf_mvt),
    data = data,
    chains = 4, iter = 4000, warmup = 2000,
    prior = prior,
    backend = "cmdstanr", seed = 42,
...
Looking at the summary, we find that most fixed effects results are largely the same, and importantly, that the qualitative interpretations of the individual-level correlations between nematode and the other traits remain unchanged. Some of the code below can be repurposed, with slight tweaks, if the reader want to check these statements in more details.

## 2F - estimating repeatabilities
...
varcomps_mvt <- tibble(
  ...
  mutate(repeatability = VI/VP)

mean_hdi(varcomps_mvt$repeatability)
...
mean_hdi(varcomps_food$repeatability)
```
...
preds_size <- data %>%
  ...
  mutate(.epred = (.epred * sd_size) + mean_size) # we back-transform
```
...
preds_size %>%
  ...
  group_by(population) %>%
  compare_levels(variable = .epred, by = band_number) %>%
  mean_hdi()
...
preds_size %>%
  ...
  compare_levels(variable = .epred, by = population) %>%
  mean_hdi()
```
...
(NB: no Figure 2 from code; it is made of photographs)


This figure is made of three parts: one for the encapsulated nematodes (no stats, no counts, we just want to have an idea about presence), one for the live nematodes, one for the live trematodes.

### Encapsulated nematodes

That subfigure is just the presence/absence of encapsulated nematodes in shells. We didn't do stats, and we don't use counts, for the same reasons: contrary to live parasites that can be cleared, encapsulated parasites accumulate with age. So unless we know snails' age, which we don't, we can't do meaningful quantitative analyses. But having a broad qualitative idea of how many adults have had an encounter is useful:
...
p_nema <- ggplot(subset(data, dead == 0 & session == 1) %>%
  ...
  stat_eye(
    data = preds_nematode,
    aes(band_number, .epred, fill = band_number),
    .width = c(0.001, 0.95), slab_alpha = 0.7,
    point_interval = mean_hdi
  ) +
  scale_y_continuous("Number of live nematodes") +
  scale_x_discrete("", labels = c("0 bands", "3 bands", "5 bands")) +
  scale_fill_manual(values = c("#fe9929", "#d95f0e", "#993404")) +
  facet_wrap(~population)
```
...
p_trema <- ggplot(subset(data, dead == 0 & session == 1) %>%
  ...
  stat_eye(
    data = preds_trematode,
    aes(band_number, .epred, fill = band_number),
    .width = c(0.001, 0.95), slab_alpha = 0.7,
    point_interval = mean_hdi
  ) +
  scale_y_continuous("Number of live trematodes") +
...
We basically do the same for this figure about behaviour results, with one nuance. Since behaviour were measured twice, we need a way to link observations from the same individual together
...
p_mvt <- ggplot(subset(data, is_active == TRUE) %>%
  mutate(population = fct_recode(population,
    `shaded habitat (no nematodes)` = "shaded",
    `open habitat (nematodes)` = "sun_exposed"
  ))) +
  geom_line(aes(
    as.numeric(factor(band_number)) + 0.5 * session - 0.75, ## this is some taped-together solution to draw a line connecting the points together
    scale_mvt,
    group = fullID
  ),
  col = "grey"
  ...
  scale_fill_manual(values = c("#fe9929", "#d95f0e", "#993404")) +
  scale_y_continuous("Movement (1/latency to leave, scaled)") +
  scale_x_discrete("", labels = c("0 bands", "3 bands", "5 bands")) +
  facet_wrap(~population)
```
...
p_food <- ggplot(data %>%
  ...
  geom_point(
    aes(band_number, Food_intake, group = session),
    position = position_dodge(width = 1), col = "grey"
  ) +
  geom_line(
    aes(as.numeric(factor(band_number)) + 0.5 * session - 0.75,
      Food_intake,
      group = fullID
    ),
    col = "grey"
  ...
  scale_fill_manual(values = c("#fe9929", "#d95f0e", "#993404")) +
  scale_y_continuous("Food intake (g)") +
  scale_x_discrete("Shell phenotype (band number)",
    labels = c("0 bands", "3 bands", "5 bands")
  ) +
  facet_wrap(~population)
...
And just like with parasites, we can do posterior comparisons (here they're differences)

```{r behaviour-comparisons}
...
Table 1 displays the fixed effect coefficients values, and the random effects/ residual SDs. We do some cleaning/ordering so that the output can be directly copied into the markdown file of the manuscript

First we create a general summary table that is easier to interrogate than the summary() object, and that uses HDIs as intervals rather than quantile intervals:

```{r general-summary}
summary_mod <- mod_multi %>%
  as_draws_df() %>%
  as_tibble() %>%
...
summary_mod %>%
  print(n = Inf)
```
(we can here easily replace `model_multi` by `model_multi2` if we want to see whether this summary changes when we re-include the shaded population in the nematode sub-model. Answer: not really)


Then we use it to built Table 1
```{r table1}
table1 <- summary_mod %>%
  mutate(value = paste(round(value, 2) %>% format(nsmall = 2),
    " [", round(.lower, 2) %>% format(nsmall = 2),
    "; ",
    round(.upper, 2) %>% format(nsmall = 2),
    "]",
    sep = ""
  )) %>%
  ...
  select(
    coefficient,
    `Nematode abundance`, `Trematode abundance`,
    `Movement`, `Food intake`
  ) %>%
  unnest(cols = c())
...
And we use the correlation component of the summary for table 2
...
Here it's a bit harder to get a near-manuscript ready table (because the response1 response2 doesn't match the rows/columns that would make the most sense). But the table is small, so it's easy to copy paste the contents of the `value` column back to the manuscript markdown file

## Some other useful outputs

Here's a plot that places the snail excluded because it died (in red) within the size distribution of all the other snails
...
It was smaller than average, but not exceptionally so

### Are the uninfected from both populations similar in their behaviours?

During review, a question came up: how can we be sure that differences in behaviours between populations are due to parasites? The answer is that, we cannot know for sure with this simple dataset, especially since no snails at all were infected by nematodes in the shaded population, but the data support that hypothesis well. In addition to the models presented, an exploratory* way too look at it would be to re-run the behaviour models by using only uninfected snails (* exploratory because doing that would remove a lot of the snails from the dataset, and in a biased way):

```{r add-binary-infection-status}
data_x <- data_parasite %>% 
  mutate(has_nematodes=Nematode_live_all>0) %>%
  select(fullID,has_nematodes)

data_y <- data %>% left_join(data_x)
```


```{r uninfec-only-models}
mod_mvt_uninfec <- brm(
  bf(scale_mvt | subset(is_active == TRUE & has_nematodes==FALSE) ~ ## we exclude non-moving snails
       0 + band_number + band_number:population +
       scale_size + (1 | series) + (1 | fullID)),
  data = data_y, chains = 4, iter = 2000, warmup = 1000,
  prior = c(
    set_prior("normal(0,1)", class = "b"),
    set_prior("normal(0,1)", class = "sd"),
    set_prior("normal(0,1)", class = "sigma")
...
mod_food_uninfec <- brm(
  bf(scale_food | subset( has_nematodes==FALSE)~
       0 + band_number + band_number:population +
       scale_size + (1 | series) + (1 | fullID)),
  data = data_y, chains = 4, iter = 4000, warmup = 2000,
  prior = c(
    set_prior("normal(0,1)", class = "b"),
    set_prior("normal(0,1)", class = "sd")
  ),
...
summary(mod_mvt_uninfec)
summary(mod_food_uninfec)
```

We can then build predictions to determine whether using only uninfected snails cancel population differences in morph responses. keep in mind however that due to the aforementioned biases introduced by removing infected individuals, this can only be a qualitative exploratory check

```{r build-predictions-uninfec-models}
preds_mvt_y <- data_y %>%
  select(population, band_number, mean_mvt, sd_mvt) %>%
  distinct() %>%
  mutate(scale_size = 0, session = 1, is_active = TRUE, has_nematodes=FALSE) %>%
  add_epred_draws(mod_mvt_uninfec, re_formula = NA) %>%
  ungroup() %>%
  mutate(population = fct_recode(population,
                                 `shaded habitat (no nematodes)` = "shaded",
                                 `open habitat (nematodes)` = "sun_exposed"
...
preds_food_y <- data_y %>%
  ...
  distinct() %>%
  mutate(scale_size = 0, session = 1, is_active = TRUE, has_nematodes=FALSE) %>%
  add_epred_draws(mod_food_uninfec, re_formula = NA) %>%
  ungroup() %>%
  mutate(population = fct_recode(population,
                                 `shaded habitat (no nematodes)` = "shaded",
                                 `open habitat (nematodes)` = "sun_exposed"
...
```{r comparisons-uninfec-models}
preds_mvt_y %>%
  group_by(population) %>%
  compare_levels(.epred, by = band_number) %>%
  mean_hdi() %>% 
  print(n=Inf)
...
preds_mvt_y %>%
  group_by(band_number) %>%
  compare_levels(.epred, by = population) %>%
  mean_hdi() %>% 
  print(n=Inf)
