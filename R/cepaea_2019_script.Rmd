...
```
 ...
 Total body mass was additionally recorded (see **Data availability**) but was not used in further analyses due to the high within-individual variability of water content [@howesWaterRelationsSnails1934] and the *a priori* unknown contributions of parasites to total mass.
...
data<- data  %>% 
  ...
  mutate(scale_size = scale(shell_diameter)[,1],
...
```{r}
#number of nematodes in the dead individual
subset(data_parasite$Nematode_live_all,data_parasite$dead==1)
#
subset(data_parasite$Trematode_kidney,data_parasite$dead==1)


subset(data_parasite$Acari_live_all,data_parasite$Acari_live_all > 0)

subset(data_parasite$Acari_encaps,data_parasite$Acari_encaps > 0)

table(paste("has encaps", data_parasite$Nematode_encaps_all>0), paste("has live nematodes", data_parasite$Nematode_live_all>0) )
...
data_parasite %>% 
  ...
  summarise(preva_trema=mean(Trematode_kidney>0),
...
data_parasite %>% 
  ...
  summarise(N=length(Nematode_encaps_all),
