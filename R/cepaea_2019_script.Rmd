...
data <- data %>% 
  ...
  mutate(latency=replace(latency,which(has_moved == FALSE), NA),
         mvt = replace(mvt,which(has_moved == FALSE),0)) %>% 
  # put a zero mvt/ NA latency for the ones that don't move, they will be excluded in-model anyway
...
mod_size <-brm(bf(scale_size ~ is_pop_shaded * (band_number + fusion)),
...
mod_food_test_beta <- brm(
    bf(food_test/1.5|cens(- 1*(food_test<=0.0001)) ~  # we use left-censoring to allow zero values in a Beta model
         ...
         is_pop_shaded * (band_number + fusion) + 
...
mod_mvt_test_lognormal <- brm(
  bf(latency|subset(has_moved==TRUE) + ## we exclude non-moving snails 
         cens(FPT_10cm>=20) ~ is_pop_shaded * (band_number + fusion) + 
...
```{r}
pp_check(mod_mvt_test_lognormal,"stat_2d")
```
same as with food, seems off (not shown, but easy to change from lognormal to Gamma, or other survival-data adapted families, and see they have the same problems)

let's try the Weibull (gamma (not shown): better than lognormal but worse than weibull)

```{r}

mod_mvt_test_weibull <- brm(
  bf(latency|subset(has_moved==TRUE) + ## we exclude non-moving snails 
         cens(FPT_10cm>=20) ~ is_pop_shaded * (band_number + fusion) + 
    scale_size + (1|series)+(1|fullID)), family=weibull(link_shape="log"),
    data=data, chains=4,iter=2000,warmup=1000,
    prior=c(set_prior("normal(0,1)",class="Intercept"), # priors are probably bad but this is just for testing
                set_prior("normal(0,1)",class="b"),
                set_prior("normal(0,1)", class="sd"),
