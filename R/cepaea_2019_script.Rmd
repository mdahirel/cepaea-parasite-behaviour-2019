...
```{r}
preds_mvt <- data %>% 
  select(population,band_number, is.shaded,mean_mvt,sd_mvt) %>% 
  distinct() %>% 
...
p_mvt <- ggplot(subset(data,has.moved==TRUE)) +
    geom_point(aes(band_number,mvt,col=population,group=session),position=position_dodge(width=1))+
    geom_line(aes(as.numeric(factor(band_number))+0.5*session-0.75,mvt,col=population,group=fullID))+
    stat_eye(data=preds_mvt,aes(band_number,.value,fill=population),
...
preds_food <- data %>% 
  select(population,band_number, is.shaded,mean_food,sd_food) %>% 
  distinct() %>% 
...
p_food <- ggplot(data) +
    geom_point(aes(band_number,Food_intake,col=population,group=session),position=position_dodge(width=1))+
    geom_line(aes(as.numeric(factor(band_number))+0.5*session-0.75,Food_intake,col=population,group=fullID))+
    stat_eye(data=preds_food,aes(band_number,.value,fill=population),
           .width=c(0.01,0.95),slab_alpha=0.7)+
  scale_y_continuous("Food intake (g/night)")+
  scale_x_discrete("Shell phenotype (band number)")+
    facet_wrap(~population)

p_mvt/p_food & theme(legend.position="none")

preds_nematode <- data %>% 
  select(band_number) %>% 
  distinct() %>% 
  mutate(scale_size=0, session=1,fusion="no", population="sun_exposed") %>% 
...
p_nema <- ggplot(subset(data_parasite,dead==0)) +
  geom_jitter(aes(band_number,Nematode_live_all,col=population),height=0)+
  stat_eye(data=preds_nematode, aes(band_number,.value),slab_alpha=0.7)+
...
p_trema <- ggplot(subset(data_parasite,dead==0)) +
  geom_jitter(aes(band_number,Trematode_kidney,col=population),height=0)+
  stat_eye(data=preds_trematode, aes(band_number,.value),slab_alpha=0.7)+
  scale_y_continuous("Trematodes in kidney") +
  scale_x_discrete("Shell phenotype (band number)")+
  facet_wrap(~population)

p_nema/p_trema & theme(legend.position="none")
