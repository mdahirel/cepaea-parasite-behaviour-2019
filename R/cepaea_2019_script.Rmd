...
data <- left_join(raw_behaviour,data_parasite) %>% 
  ...
  #we then standardize covariates and responses
...
FPT1 is the time to initiate activity, so our movement time is FPT3 - FPT1, and any snail that has FPT1==1200 did not move, so no valid movement value
...
let's try to fit a simple model with this movement variable. movement data is time, so a lognormal model should be right-ish, no?:

```{r}
mod_mvt_test <- brm(
  bf(mvt|subset(has.moved==TRUE) + ## we exclude non-moving snails 
         cens(mvt>=1200) ~ is.shaded * (band_number + fusion) + 
...
fairly easy to see that this model is rubbish and doesn't even come close to capturing the very slow individuals, even with individual-level random effects. Changing distributions (not shown) does not solve the problem. Let's try something else, by inverting the movement variable, so that it becomes something closer to a speed:

```{r}
data_test$mvt2=1/data_test$mvt

mod_mvt_test2 <- brm(
  bf(mvt2|subset(has.moved==TRUE) + ## we exclude non-moving snails 
         cens(- 1*(mvt>=1200)) ~ # because the mvt metric is inversed, the censure is too (right- to left-) 
